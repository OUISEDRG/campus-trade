...
-- 尝试添加 openid 字段
-- 先检查字段是否存在，避免错误
ALTER TABLE `user` ADD COLUMN IF NOT EXISTS `openid` VARCHAR(100) UNIQUE COMMENT '微信OpenID' AFTER `status`;

ALTER TABLE `user` ADD COLUMN IF NOT EXISTS `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间' AFTER `openid`;

-- 给 goods 表添加字段
ALTER TABLE `goods` ADD COLUMN IF NOT EXISTS `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间' AFTER `is_deleted`;

-- 给 orders 表添加字段
ALTER TABLE `orders` ADD COLUMN IF NOT EXISTS `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间' AFTER `status`;

INSERT IGNORE INTO `message_template` (`template_code`, `template_name`, `title`, `content`, `type`, `status`, `variables`) VALUES
